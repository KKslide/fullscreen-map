<template>
  <div id="map" ref="map" :style="this.childClass">
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      list:[{name: "新疆",coord:[86.61 , 40.79]}],
      title :'',
  ...
  mounted() {
    ...
    setTimeout(()=>{
			...
			//console.log(this.nationMapValueData);
		 	this.getMap();
		  this.dataList();
		},800)
...
//							console.log(that.nationMapValueData[i])
							that.title = '交易情况：'+that.nationMapValueData[i].type + " "+that.nationMapValueData[i].amount
							name = that.nationMapValueData[i].type
						}
					}
...
//           itemHeight: '80%',
//           align: 'top',
//           color: ['rgba(254,255,1,0.6)', '#50a3ba', '#5A3A92'],
//           textStyle: {
//             color: '#fff',
//             fontSize: '80%'
//           },
