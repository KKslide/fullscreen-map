	...
	<div class="SemicircleProgressBar" >
		<div v-for="(item,index) in list" :key="index" class="SemicircleProgressBarWarp" >
			...
			<div class="whz">
					...
					<div class="wbhz"></div>
					<div class="d1" :style="{'border':item.pro,'border-bottom':'none'}"><span class="dwy"></span>
				</div>
			</div>
			<div class="SemicircleProgressBar-count">
				<span>{{showData[index]}}</span>
				<span>{{item.unit}}</span>
			</div>
		</div>
	</div>
	...
	export default {
		...
		data() {
			return {
				...
				list:[{"cout":0,"pro":".15rem solid #fc4b4b","img":"../../../static/images/money.png","title":"总贷款余额","unit":"元"},
					  {"cout":0,"pro":".15rem solid #00a2ff","img":"../../../static/images/custom.png","title":"总客户数","unit":"户"}]
			};
		},
		mounted() {
			...
			for(var j = 0;j<this.midData.length;j++){
				this.list[j].cout = this.midData[j]
			}
